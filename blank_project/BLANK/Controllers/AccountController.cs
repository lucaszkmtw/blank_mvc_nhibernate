...
namespace ECO.RCF.Controllers
{
    ...
    public class AccountController : WebSecurityController
    {
        ...
        private static string APPEstructura = "RCF"; //Variable que debe cambiar;
        private static BlankApplicationService BlankAppBussinessLogicService = BlankApplicationService.Instance;
        ...
        public ActionResult Login(string returnUrl)
        {
			...
			string nuevaVersion = "";
			//WebRequest request = WebRequest.Create(ConfigurationManager.AppSettings["WebResources"] + "Version.txt");
        ...
        public ActionResult Login(UsuarioForm model, string returnUrl)
        {
            ...
            {
                Session["token"] = BlankAppBussinessLogicService.SolicitarToken();
                ...
                if (r.LoginResult.Exception != null)//envio mail con la excepcion que devolvio SRSeguridad
                {
                    ...
                    EnviarEmail(excepcion, "AccountController", "Login");
                }
            }
            return View(model);
        }
        ...
        public ActionResult ExternalLoginService(string usuario, string returnUrl, string token)
        {
            ...
            if (r.ExternalLoginResult.Exception != null)//envio mail con la excepcion que devolvio SRSeguridad
            {
                ...
                EnviarEmail(excepcion, "AccountController", "ExternalLoginService");
            }
            return View("Login");
        }
        ...
        private void CargarContexto(string usuario, ResultadoLogin r)
        {
            ...
            //TODO cargo las actividades asociadas al usuario para el acceso a menu y Blank
            FormsAuthentication.SetAuthCookie(usuario, false);
            Session.Add("usuario", usuario);
            Session.Add("IdUsuario", r.UsuarioId);
            if (r.Avatar != null)
            ...
            Session.Add("ambienteDB", BlankAppBussinessLogicService.ObtenerNombreBaseDeDatos());
        ...
        /// <returns></returns>
        public static TreeView LoadTreeMenu(List<MenuWS> usuariosMenu)
        {
            TreeView tvMenu = new TreeView();
            if (usuariosMenu != null && usuariosMenu.Count > 0)
            {
                foreach (MenuWS menu in usuariosMenu.Where(m => m.IdPadre == null).ToList())
                {
        ...
        /// <summary>
        /// Método que comunica la actividad del usuario a Blank/Monitor
        /// </summary>
        ...
        private void RegistarAuditoriaConexion()
        {
            ...
            //Instancia del Cliente de Blank
            LibreriaSeguridadClient client = new LibreriaSeguridadClient();
            //Carga del request de auditoria
            RegistarAuditoriaConexionRequest request = new RegistarAuditoriaConexionRequest();
        ...
        private void RegistarAuditoriaDesconexion()
        {
            ...
            LibreriaSeguridadClient client = new LibreriaSeguridadClient();
            //Carga del request de auditoria
            RegistarAuditoriaDesconexionRequest request = new RegistarAuditoriaDesconexionRequest();
            if (Session["avatar"] != null)
                request.avatar = (byte[])Session["avatar"];
            else
                request.avatar = null;
